...
if(isset($_POST['search'])){
  ...
  $D=$_POST['District'];
  if($t=="Covid_icu"){
  $query1 = "SELECT * FROM covid_icu WHERE availability IN (SELECT availability FROM covid_icu WHERE availability=\"yes\" OR availability=\"booked\") AND district='$D' ORDER BY hospital_name";
  echo "COVID ICU List:<br><br>";
  }
  else if($t="General_icu"){
  $query1 = "SELECT * FROM general_icu WHERE availability IN (SELECT availability FROM covid_icu WHERE availability=\"yes\" OR availability=\"booked\") AND district='$D' ORDER BY hospital_name";
  ...
  if($result1->num_rows>0){
    ...
    <input type="text" name="Street_No" size="15"value="<?php echo 'Street_No'?>">  
    <input type="text"style=margin-bottom:30px name="Book" size="6"value="<?php echo 'Book Bed'?>"> 
    </form>
    <?php
    ...
    <input type="text"name="Street_No"size="15" value="<?php echo $row['Street_No']  ?>">
    <input type="submit"  style=margin-bottom:30px name="Confirm" size="15" value="Confirm">
    </form>
    <?php    
